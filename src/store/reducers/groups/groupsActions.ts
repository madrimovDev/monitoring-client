...
export const getAllGroups = createAsyncThunk(
  ...
  async (_, {rejectWithValue}) => {
    try {
      ...
      const response = await api.get<Groups.GroupsResponse>(
        `/organizations/${ogrID}/${URL}`,
      );
...
>('groups/create', async (group, {rejectWithValue}) => {
  try {
    const ogrID = await getUserDataFromLocalStorage('organizationId');
    if (ogrID === null) throw new Error('Organizations id not found');
    const response = await api.post<Groups.CreatedGroupResponse>(
      `/organizations/${ogrID}/${URL}`, group
    );
    return {
...
export const updateGroup = createAsyncThunk<
  ...
  Groups.NewGroup,
  {rejectValue: string}
>('groups/update', async (group, {rejectWithValue}) => {
  try {
    const ogrID = await getUserDataFromLocalStorage('organizationId');
    if (ogrID === null) throw new Error('Organizations id not found');
    const response = await api.put<Groups.CreatedGroupResponse>(
      `/organizations/${ogrID}/${URL}`, group
    );
    return {
      message: response.data.message,
